...
## Package Application
PACKNAME=demo_Application
if [ -d ${PACKAGE}/${PACKNAME} ]; then
	rm -rf ${PACKAGE}/${PACKNAME}
fi
mkdir -p ${PACKAGE}/${PACKNAME}
MF=${PACKAGE}/${PACKNAME}/Makefile

echo '# BiscuitOS Application' >> ${MF}
echo '#' >> ${MF}
...
echo '' >> ${MF}
echo "ROOT=${OUTPUT}" >> ${MF}
echo "CROSS_NAME=${CROSS_TOOL}" >> ${MF}
echo 'CROSS_PATH=$(ROOT)/$(CROSS_NAME)/$(CROSS_NAME)/bin' >> ${MF}
echo 'CROSS_TOOLS=$(CROSS_PATH)/$(CROSS_NAME)-' >> ${MF}
echo 'PACKAGE=$(ROOT)/RunBiscuitOS.sh' >> ${MF}
echo '' >> ${MF}
echo 'CC=$(CROSS_TOOLS)gcc' >> ${MF}
echo 'NM=$(CROSS_TOOLS)nm' >> ${MF}
echo 'CPP=$(CROSS_TOOL)g++' >> ${MF}
echo '' >> ${MF}
echo '# Install path' >> ${MF}
echo 'INSTALL_PATH=$(ROOT)/rootfs/rootfs/usr/bin' >> ${MF}
echo '' >> ${MF}
echo '# SRC' >> ${MF}
echo 'SRC += demo.c' >> ${MF}
echo '' >> ${MF}
...
echo '' >> ${MF}
echo '$(TARGET): $(SRC)' >> ${MF}
echo -e '\t@$(CC) $(CFLAGS) $^ -o $@' >> ${MF}
echo -e '\t$(info "Compile .... [OK]")' >> ${MF}
echo '' >> ${MF}
echo 'install:' >> ${MF}
echo -e '\t@cp -rf $(TARGET) $(INSTALL_PATH)' >> ${MF}
echo -e '\t$(info "Install .... [OK]")' >> ${MF}
echo '' >> ${MF}
echo 'pack:' >> ${MF}
echo -e '\t@$(PACKAGE) pack' >> ${MF}
echo -e '\t$(info "Pack    .... [OK]")' >> ${MF}
echo '' >> ${MF}
echo 'clean:' >> ${MF}
echo -e '\t@rm -rf $(TARGET) *.o' >> ${MF}
echo -e '\t$(info "Clean   .... [OK]")' >> ${MF}
echo '' >> ${MF}
echo '' >> ${MF}
echo '# Reserved by BiscuitOS :)' >> ${MF}

MF=${PACKAGE}/${PACKNAME}/demo.c
...
echo ' * published by the Free Software Foundation.' >> ${MF}
echo ' */' >> ${MF}
echo '#include <stdio.h>' >> ${MF}
echo '#include <stdlib.h>' >> ${MF}
echo '' >> ${MF}
echo 'int main()' >> ${MF}
echo '{' >> ${MF}
echo '	printf("Hello BiscuitOS!\n");' >> ${MF}
echo '	return 0;' >> ${MF}
echo '}' >> ${MF}

## Package Dynamic Library
PACKNAME=demo_dynamic_library
if [ -d ${PACKAGE}/${PACKNAME} ]; then
	rm -rf ${PACKAGE}/${PACKNAME}
fi
mkdir -p ${PACKAGE}/${PACKNAME}
...
echo '# BiscuitOS Dynamic library' >> ${MF}
echo '#' >> ${MF}
echo '# (C) 2019.08.16 BiscuitOS <buddy.zhang@aliyun.com>' >> ${MF}
echo '#' >> ${MF}
echo '# This program is free software; you can redistribute it and/or modify' >> ${MF}
...
echo 'PATH += :$(CROSS_PATH)' >> ${MF}
echo '' >> ${MF}
echo '# Package information' >> ${MF}
echo 'PACKAGE := ncurses-6.1.tar.gz' >> ${MF}
echo 'BASENAM := ncurses-6.1' >> ${MF}
echo 'TARCMD  := tar -xvf' >> ${MF}
echo 'URL     := http://ftp.gnu.org/pub/gnu/ncurses/' >> ${MF}
echo 'CONFIG  := --prefix=$(INSTALL_PATH) --host=$(CROSS_NAME)' >> ${MF}
echo 'CONFIG  += --disable-stripping --with-shared' >> ${MF}
echo '' >> ${MF}
echo 'all:' >> ${MF}
