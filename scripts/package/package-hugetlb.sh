...
MF=${PACKAGE_ROOT}/${PACKAGE_NAME}/Makefile
...
echo '' >> ${MF}
echo 'download:' >> ${MF}
echo -e '\tgit clone $(GITHUB) $(BASENAME) -b v$(VERSION)' >> ${MF}
echo '' >> ${MF}
echo 'tar:' >> ${MF}
...
echo '' >> ${MF}
echo 'install:' >> ${MF}
echo -e '\tsudo chmod 755 $(shell pwd)/$(BASENAME)/RunBiscuitOS.sh' >> ${MF}
echo -e '\tsudo cp -rfa $(shell pwd)/$(BASENAME)/RunBiscuitOS.sh $(INS_PATH)/bin/' >> ${MF}
echo -e '\t@if [ "${BS_SILENCE}X" != "trueX" ]; then \' >> ${MF}
echo -e '\t\tfiglet "BiscuitOS" ; \' >> ${MF}
echo -e '\tfi' >> ${MF}
...
MF=${PACKAGE_ROOT}/${PACKAGE_NAME}/README.md
