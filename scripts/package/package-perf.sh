...
if [ ${ARCH} == "i386" -o ${ARCH} == "x86_64" ]; then
        echo -e '\tmake  ARCH=$(ARCH) tools/perf -j98 || exit 1 ;\' >> ${MF}
else
        echo -e '\tmake  ARCH=$(ARCH) CROSS_COMPILE=$(CROSS_TOOL) tools/perf -j84 || exit 1 ;\' >> ${MF}
fi
echo -e '\tcd - > /dev/null' >> ${MF}
echo '' >> ${MF}
echo 'menuconfig:' >> ${MF}
...
echo -e '\tsudo cp $(ROOT)/linux/linux/tools/perf/perf $(INS_PATH)/bin/' >> ${MF}
echo -e '\t@if [ "${BS_SILENCE}X" != "trueX" ]; then \' >> ${MF}
echo -e '\t\tfiglet "BiscuitOS" ; \' >> ${MF}
echo -e '\tfi' >> ${MF}
