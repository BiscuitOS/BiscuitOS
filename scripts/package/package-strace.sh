...
echo -e '\tPKG_CONFIG_PATH=$(DPK_PATH) \' >> ${MF}
if [ ${ARCH} == "i386" ]; then
	echo -e '\t./configure $(CONFIG) CC="gcc -m32" LD=$(CROSS_TOOL)ld' >> ${MF}
elif [ ${ARCH} == "x86_64" ]; then
	echo -e '\t./configure $(CONFIG) CC=gcc LD=$(CROSS_TOOL)ld' >> ${MF}
else
	echo -e '\t./configure $(CONFIG) CC=$(CROSS_TOOL)gcc LD=$(CROSS_TOOL)ld' >> ${MF}
fi
echo -e '\t@if [ "${BS_SILENCE}X" != "trueX" ]; then \' >> ${MF}
echo -e '\t\tfiglet "BiscuitOS" ; \' >> ${MF}
echo -e '\tfi' >> ${MF}
