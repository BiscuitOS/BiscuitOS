...
# X86-64 GDB helper scripts.
#
# (C) 2022.05.12 BiscuitOS <buddy.zhang@aliyun.com>
#
...
my $VM_FILE="$ROOT/linux/linux/arch/x86/boot/compressed/vmlinux";
my $VM_KERN="$ROOT/linux/linux/vmlinux";

## Auto objdump vmlinux file
`objdump -x $VM_FILE > $GDB_FILE`;
`readelf -S $VM_KERN > $GDB_FIL2`;

my $restart=hex(`cat $GDB_FILE | grep " restart" | awk '{print \$1}'`);
my $Image=`ls -l $ROOT/linux/linux/arch/x86/boot/Image | awk '{print \$5}'`;
...
my $gdb_boot="$ROOT/package/gdb/gdb_boot";
...
## Create gdb_boot
my $content_string="add-symbol-file $ROOT/linux/linux/arch/x86/boot/setup.elf 0x0000";
open FH, ">>$gdb_boot";
print FH "# Debug bootloader for kernel\n";
print FH "#\n";
...
print FH " \n";
print FH "# setup architecture\n";
print FH "#\n";
print FH "set architecture i386:x86-64\n";
print FH "#\n";
print FH "# Remote to gdb\n";
print FH "#\n";
print FH "target remote :1234\n";
print FH "\n";
print FH "# Load bootloader for kernel\n";
print FH "#\n";
print FH "$content_string\n";
close FH;

## Create gdb_zIamge
my $content_string="add-symbol-file $ROOT/linux/linux/arch/x86/boot/compressed/vmlinux 0x00000";
open FH, ">>$gdb_zImage";
...
## Create gdb_RzIamge
$content_string=sprintf "add-symbol-file $ROOT/linux/linux/arch/x86/boot/compressed/vmlinux %#x", $final;
open FH, ">>$gdb_RzImage";
