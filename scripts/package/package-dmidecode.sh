...
echo 'install:' >> ${MF}
echo -e '\t@cd $(BASENAME) ; \' >> ${MF}
echo -e '\tsudo cp biosdecode dmidecode ownership vpddecode $(INS_PATH)/bin/ ;' >> ${MF}
echo -e '\t@if [ "${BS_SILENCE}X" != "trueX" ]; then \' >> ${MF}
echo -e '\t\tfiglet "BiscuitOS" ; \' >> ${MF}
echo -e '\tfi' >> ${MF}
...
if [ ${ARCH} == "i386" -o ${ARCH} == "x86_64" ]; then
	echo -e '\tmake  ARCH=$(ARCH) bzImage -j80 ;\' >> ${MF}
else
	echo -e '\tmake  ARCH=$(ARCH) CROSS_COMPILE=$(CROSS_TOOL) -j80 ;\' >> ${MF}
fi
