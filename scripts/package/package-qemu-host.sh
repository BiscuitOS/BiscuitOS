...
echo '' >> ${MF}
echo 'download:' >> ${MF}
echo -e '\tln -s $(ROOT)/qemu-system/qemu-system $(BASENAME)' >> ${MF}
echo -e '\t@if [ "${BS_SILENCE}X" != "trueX" ]; then \' >> ${MF}
echo -e '\t\tfiglet "BiscuitOS" ; \' >> ${MF}
echo -e '\tfi' >> ${MF}
...
echo "set args -m 64M -kernel ${OUTPUT}/linux/linux/${KERNEL_IMAGE} -hda ${OUTPUT}/BiscuitOS.img -drive file=${OUTPUT}/Freeze.img,if=virtio -nographic -append \"root=/dev/sda rw rootfstype=ext4 console=ttyS0 init=/linuxrc loglevel=8\"" >> ${MF}
